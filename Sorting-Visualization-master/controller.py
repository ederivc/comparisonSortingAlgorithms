...
def draw_info(comp_value: int, color: list, canva: Canvas, _list: list) -> None:
    ...
    for key, value in enumerate(_list):
        ...
        canva.canvas.create_text(x, value[1]+20, anchor="sw", text=value[0], fill="black")
        canva.canvas.create_text(400, 580, anchor="sw", text="Comparisons", fill="black")
        canva.canvas.create_text(400, 600, anchor="sw", text=comp_value, fill="black")
        x += 40
        y += 40

def final_animation(comp_value: int, canva: Canvas , _list: list, time) -> None:
    for _ in range(len(_list)):
        draw_info(comp_value, ["#7734d8"  for i in range(len(_list))], canva, _list)
...
def select_algorithm(speed: float, algorithm: list, random_list: list, dicc: dict) -> None:
    ...
    GUI.list_1._list = random_list.copy()
    GUI.list_2._list = random_list.copy()
    GUI.list_3._list = random_list.copy()

    threads.start_thread_animation(algorithm1, algorithm2, algorithm3)
