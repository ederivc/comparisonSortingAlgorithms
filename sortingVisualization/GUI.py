...
class CreateImage(PhotoImage):
    def __init__(self) -> None:
        super().__init__()
        self.dark_image = PhotoImage(file = "/home/ederivc/Documents/test/Sorting-Visualization-master/Sorting-Visualization-master/change_color.png")
...
class CreateButton:
    ...
    def _setup(self) -> None:
        ...
        command = lambda: controller.select_algorithm(
            self.speed_values(self.args[1][4].get()), [self.args[1][5][0].get(), 
            self.args[1][5][1].get(), self.args[1][5][2].get()], obj.random_list._list, 
            self.args[1][1]
        ))    
...
class Application:
    ...
    def generate_info(self, min_value: int, max_value: int) -> None:    
        ...
        for i in range(12):
            ...
            obj.random_list._list.append(number)

        max_value = max(obj.random_list._list)
        temp_list = []

        for i in obj.random_list._list:
        ...
        obj.random_list._list = temp_list
        print(obj.random_list._list)

        threads.start_thread(
            self.canvas_1, self.canvas_2, self.canvas_3, obj.random_list._list
        )
